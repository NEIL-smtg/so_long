...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/20 18:23:12 by suchua            #+#    #+#             */
/*   Updated: 2022/11/07 19:47:04 by suchua           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		...
		if (read_res == -1)
			return (NULL);
		line = gnl_strjoin(line, partial);
		if (ft_contain_n(line) || read_res == 0)
			break ;
	}
	free(partial);
	single = get_line(line);
	line = ft_jump(line);
	return (single);
}
